...
pub fn spigot_pi(buffer: []u8, allocator: std.mem.Allocator) !void {
...
test "spigot with umm" {
    const heapSize = std.math.maxInt(u15) * 8;
    var buffer: [heapSize]u8 align(16) = undefined;
    const Allocator = UmmAllocator(.{});
    var allocInst = try Allocator.init(&buffer);
    const allocator = allocInst.allocator();

    var result: [1000]u8 = undefined;
    try spigot_pi(&result, allocator);

    const expected = try parsePiDigits(std.testing.allocator, result.len);
    defer std.testing.allocator.free(expected);
    for (0..result.len) |i| {
        try expectEqual(expected[i], result[i]);
    }
}
